...
with st.form("input_form"):
    ...
    for v in v_cols[:6]:  # select first 6 for brevity
        mn = float(df[v].min())
        mx = float(df[v].max())
...
if submitted:
    try:
        features = [0]  # Time placeholder
        for v in v_cols[:6]:
            features.append(sliders[v])
        # fill rest with zeros
        remaining = 30 - len(features) - 1  # exclude amount
        ...
        if pred == 0:
            st.success("Transaction predicted as SAFE.")
        else:
            st.error("Transaction predicted as FRAUD.")
